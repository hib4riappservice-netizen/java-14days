...
- **Obsidian** など専用のMarkdownビューアアプリ

**IntelliJ は Day 0 でどのみち入れるので、そこで開くのが一番手間がかかりません。**

**⚠ Obsidian など「独自の設定フォルダをリポジトリ内に作るツール」を使う場合は要注意**
Obsidianは開いたフォルダの直下に `.obsidian/` という設定フォルダを自動生成します。これをそのままコミットすると、あなた個人のウィンドウ配置やテーマ設定が他人と共有され、無関係な差分ノイズが増え続けます。
`04_setup.md` の `.gitignore` ひな形にはすでに `.obsidian/` を含めていますが、**それ以外のツール（Typora、Notion のローカル同期フォルダ等）を使う場合は、そのツールの設定フォルダ名を自分で `.gitignore` に追加してください。** 「見慣れない隠しフォルダが `git status` に出てきたら、まず除外を検討する」という習慣を、Day 0 のうちに身につけておくと後で助かります。
