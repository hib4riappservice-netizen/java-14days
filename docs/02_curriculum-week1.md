...
8. **`.gitignore` を作り、`git add` → `git commit` → `git push` する**
   ...
   - **明日（Day 1）からは、この作業を毎日繰り返します。** 手順は `13_daily-commands.md` に一本化してあるので、今日のうちにブックマークしておいてください
...
## ② 新出用語
if / switch式 / 三項演算子 / for / while / 拡張for文 / 配列 / インデックス / スタックトレース / デバッガ / ブレークポイント / ステップ実行
...
**簡単な if-else は1行で書けます（三項演算子）**
```java
String mark = hours > 8 ? "★残業" : "";
```
`条件 ? Aの場合の値 : Bの場合の値` と読みます。上の例は「`hours > 8` が true なら `"★残業"`、false なら `""`」という意味です。**値を代入するだけの短い分岐にだけ使ってください。** 条件が複雑になると読みにくくなるので、その場合は普通の `if` を使います。
...
>
> **上の例の `throw new IllegalArgumentException(...)` について**：これは「不正な値が来たので、ここで処理を中断して呼び出し元にエラーを知らせる」という意味です。Day 1で見た `NullPointerException` と同じ「例外」を、**自分で意図的に発生させています**。仕組みの詳細（例外の種類、`try-catch`での受け止め方）は**Day 6で扱います**。今日は「想定外の値が来たら、こう書いて処理を止める」という定型だと理解すればOKです。
...
> **`new String(...)` について**：これは「わざわざ別の場所に、同じ中身の文字列をもう1つ作る」書き方です。`==` がfalseになることを見せるためにここだけで使っています。**実務で文字列をこう書くことはありません**（`String a = "田中";` と書きます）。`new` が正確には何をしているか（インスタンスを作る、という話）は**Day 3で扱います**。
...
## ⑥ 自己チェック
- [x] `==` と `equals` の違いを説明できる
- [x] 文字列比較に `==` を使ってはいけない理由を説明できる
- [x] 配列のインデックスが0から始まることと、その落とし穴を説明できる
- [x] 拡張for文と通常のfor文の使い分けを説明できる
- [x] **スタックトレースを見て、自分のコードのどの行が原因か特定できる**
- [x] ブレークポイントを置いてデバッガで変数の中身を確認できる
- [x] F8（ステップオーバー）と F7（ステップイン）の違いを説明できる
- [x] 空配列など「異常な入力」への備えが必要な理由を説明できる
...
> **⚠ ここまでの `Employee` クラスは、まだ実際には使えません。** `private` なフィールドに値を入れる手段（コンストラクタ）が無いからです。**「箱の形」を先に見せただけ**で、「箱に値を詰めて実際に1個作る（インスタンス化する）方法」は次の節で説明します。「なるほど、こう作るのか」は、ここでは分からなくて正常です。
...
public class Employee {
    ...
    // ① フィールド（このクラスが持つデータ）
    private final String employeeId;   // final = 一度決めたら変更不可。このように変更不可なオブジェクトを「イミュータブル（不変）」と呼ぶ
    ...
    public Employee(String employeeId, String name, String department) {
        ...
        // throw は「不正な値なので処理を中断してエラーを知らせる」という意味（Day 2 の switch式の節で既出）
...
**別のパッケージのクラスを使うときは、ファイルの先頭で `import` します。**
```java
import com.example.attendance.domain.Employee;   // 別パッケージのクラスを使う宣言
```
同じパッケージ内のクラス同士なら不要です。IntelliJ なら `Alt + Enter` で自動的に補ってくれます。
...
public class Attendance {
    ...
    public int overtimeHours() {
        return Math.max(0, workHours - 8);   // Math.max(a, b) は大きい方を返す標準メソッド
    }
...
> **`@Override` について**：すべてのクラスは `toString()` を最初から持っており（何もしないと `Employee@1b6d3586` のような読めない表示になります）、それをここで自分好みに**上書き**しています。`@Override` は「これは元からあるメソッドを書き換えたものです」という印で、この印と「オーバーライド」という言葉の正式な説明は**Day 4で扱います**。今日はこの2行をセットで書く定型として写経してください。
...
> **上の3行には、このあと学ぶ2つの書き方が出てきます。** `e -> ...` は「ラムダ式」（このすぐ後の節）、`Employee::getName` は「メソッド参照」（Stream の節）です。今は「1件あったときにする処理を渡している」とだけ読んでください。`EmployeeNotFoundException` は**Day 6で自作する例外**なので、まだ実際には動かせません（`import` してもコンパイルエラーになります）。
...
## ④ 写経ハンズオン（120分）
上記コード全てを手打ちして動かす。特に groupingBy は結果を `System.out.println` または `System.out.printf`（`String.format` と同じ書式を、そのまま `println` せずに出力できるメソッド）で表示して確認する。
...
> **`super(...)` について**：「親クラスのコンストラクタを呼ぶ」という意味です。例外メッセージを保持する仕組みは親（`RuntimeException`）が既に持っているので、自分で持たずに親に任せています。Day 4 の継承（`extends`）の実例だと考えてください。
