...
## ⑥ 自己チェック
- [ ] `int` と `double` と `String` と `boolean` の違いを説明できる
- [ ] 金額計算に `double` を使ってはいけない理由を説明できる
- [ ] `7 / 2` が 3 になる理由を説明できる
- [ ] NullPointerException が起きる原因を説明できる
- [ ] メソッドの「引数」と「戻り値」を説明できる
- [ ] `void` の意味を説明できる
- [ ] クラス名・メソッド名・変数名の命名ルールを説明できる
- [ ] 計算処理と表示処理を分けるべき理由を説明できる
